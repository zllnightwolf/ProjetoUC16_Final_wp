...
class QodeEssentialAddons_Framework_WPBakery_Translator {

	public function __construct() {
		$this->add_shortcodes();

		add_filter( 'qode_essential_addons_filter_framework_shortcode_content_html', array( $this, 'format_shortcode_content' ), 10, 2 );
	...
	public function map_shortcodes( $key, $shortcode ) {
		...
		if ( $shortcode->get_is_parent_shortcode() ) {
			...
			add_filter(
				'qode_essential_addons_filter_framework_add_shortcode_container',
...
//			case 'icons':
	...
	public function enqueue_scripts() {
		wp_enqueue_style( 'qode-framework-wpbakery', QODE_ESSENTIAL_ADDONS_ADMIN_URL_PATH . '/inc/shortcodes/translators/wpbakery/assets/css/wpbakery.css' );
	}

	function add_inline_style() {
		$shortcodes = qode_essential_addons_framework_get_framework_root()->get_shortcodes()->get_shortcodes();
		$style      = apply_filters( 'qode_essential_addons_filter_framework_add_wpbakery_inline_style', $style = '' );
...
if ( ! function_exists( 'qode_essential_addons_framework_init_wpbakery_translator' ) ) {
	/**
	 * Function that initialize page builder module
	 */
	function qode_essential_addons_framework_init_wpbakery_translator() {
		if ( qode_essential_addons_framework_is_installed( 'wpbakery' ) ) {
			new QodeEssentialAddons_Framework_WPBakery_Translator();

			if ( class_exists( 'WPBakeryShortCodesContainer' ) ) {
				$classes = apply_filters( 'qode_essential_addons_filter_framework_add_shortcode_container', array() );
	...
	add_action( 'init', 'qode_essential_addons_framework_init_wpbakery_translator', 99 );
}
