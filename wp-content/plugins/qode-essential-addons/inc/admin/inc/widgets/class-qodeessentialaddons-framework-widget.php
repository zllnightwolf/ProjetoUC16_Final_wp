...
abstract class QodeEssentialAddons_Framework_Widget extends WP_Widget {
	private $widget_base;
	private $widget_name;
	...
	public function import_shortcode_options( $params ) {
		...
		$shortcodes     = $qode_essential_addons_framework->get_shortcodes()->get_shortcodes();
		...
		if ( is_array( $this->widget_options ) && count( $this->widget_options ) ) { ?>
			...
			foreach ( $this->widget_options as $option ) {
				...
				if ( ! empty( $option['dependency'] ) ) {
					...
					$show = array_key_exists( 'show', $option['dependency'] ) ? qode_essential_addons_framework_return_widget_dependency_options_array( $instance, $option['dependency']['show'], true ) : array();
					$hide = array_key_exists( 'hide', $option['dependency'] ) ? qode_essential_addons_framework_return_widget_dependency_options_array( $instance, $option['dependency']['hide'], false ) : array();

					$class[]         = qode_essential_addons_framework_return_dependency_classes( $show, $hide );
					$dependency_data = qode_essential_addons_framework_return_dependency_data( $show, $hide );
				}
				...
				if ( ! empty( $option['title'] ) || ( isset( $option['name'] ) && ! empty( $option['name'] ) ) ) {
					...
					$class   = implode( ' ', $class );
					?>
					<p class="<?php echo esc_attr( $class ); ?>" <?php echo qode_essential_addons_framework_get_inline_attrs( $dependency_data, true ); ?>>
						...
						if ( isset( $option['name'] ) && ! empty( $option['name'] ) ) {
							...
							$field = new QodeEssentialAddons_Framework_Field_Mapper( $option );
							$field->render();
						}
						?>
						<?php if ( ! empty( $option['description'] ) ) : ?>
							<span class="qodef-field-description"><?php echo qode_essential_addons_framework_wp_kses_html( 'content', $option['description'] ); ?></span>
						<?php endif; ?>
					</p>
				<?php } ?>
			<?php } ?>
		<?php } else { ?>
			<p><?php esc_html_e( 'There are no options for this widget.', 'qode-essential-addons' ); ?></p>
			<?php
		}
	}
	...
	public function widget( $args, $instance ) {
		...
		$atts              = qode_essential_addons_framework_map_shortcode_fields( $this->get_option_atts(), $instance );
		...
		echo qode_essential_addons_framework_wp_kses_html( 'html', $this->render( $atts ) );
