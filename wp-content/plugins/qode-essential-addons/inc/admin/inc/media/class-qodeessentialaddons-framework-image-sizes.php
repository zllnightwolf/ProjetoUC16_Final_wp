...
if ( ! class_exists( 'QodeEssentialAddons_Framework_Image_Sizes' ) ) {
	class QodeEssentialAddons_Framework_Image_Sizes {
		...
		function __construct() {
			...
			add_filter( 'qode_essential_addons_filter_framework_pool_list_image_dimension', array( $this, 'register_pool_list_image_dimension' ) );
		}

		function populate_image_sizes() {
			do_action( 'qode_essential_addons_action_framework_before_images_register' );

			$this->image_sizes = apply_filters( 'qode_essential_addons_filter_framework_populate_image_sizes', array() );
		}
		...
		public function post_type_slug_input( $args ) {
			...
			?>
			<fieldset>
				<legend class="screen-reader-text"><span><?php esc_html_e( 'Large size', 'qode-essential-addons' ); ?></span>
				</legend>
				<label for="<?php echo esc_attr( $width_field_name ); ?>"><?php esc_html_e( 'Max Width', 'qode-essential-addons' ); ?></label>
				<input name="<?php echo esc_attr( $width_field_name ); ?>" type="number" step="1" min="0"
					   id="<?php echo esc_attr( $width_field_name ); ?>"
					   value="<?php echo esc_attr( $default_width ); ?>" class="small-text">
				<br>
				<label for="<?php echo esc_attr( $height_field_name ); ?>"><?php esc_html_e( 'Max Height', 'qode-essential-addons' ); ?></label>
				<input name="<?php echo esc_attr( $height_field_name ); ?>" type="number" step="1" min="0"
					   id="<?php echo esc_attr( $height_field_name ); ?>"
					   value="<?php echo esc_attr( $default_height ); ?>" class="small-text">
			</fieldset>
			<input name="<?php echo esc_attr( $crop_field_name ); ?>" type="checkbox"
				   id="<?php echo esc_attr( $crop_field_name ); ?>"
				   value="1" <?php echo esc_attr( $default_checked ); ?>>
			<label for="<?php echo esc_attr( $crop_field_name ); ?>"><?php esc_html_e( 'Crop to exact dimensions (normally are proportional) ', 'qode-essential-addons' ); ?></label>
			<?php
		}
