...
abstract class QodeEssentialAddons_Framework_Custom_Post_Type {
	...
	public function get_slug() {
		$slugs             = get_option( 'qode_essential_addons_framework_permalinks' );
	...
	private function add_taxonomy( QodeEssentialAddons_Framework_Custom_Post_Type_Taxonomy $taxonomy ) {
	...
	function add_post_taxonomy( $params ) {
		if ( isset( $params['base'] ) && ! empty( $params['base'] ) ) {
			...
			$taxonomy                 = new QodeEssentialAddons_Framework_Custom_Post_Type_Taxonomy( $params );
	...
	public function settings_init() {
		if ( $this->get_public() ) {
			add_settings_field(
				...
				$this->get_name() . ' ' . esc_html__( 'base', 'qode-essential-addons' ),
