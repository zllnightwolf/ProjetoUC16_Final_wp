...
abstract class QodeEssentialAddons_Framework_Section implements QodeEssentialAddons_Framework_Tree_Interface, QodeEssentialAddons_Framework_Child_Interface {
	private $scope;
	private $type;
	private $name;
	private $layout;
	...
	function render() {
		...
		if ( ! empty( $this->dependency ) ) {
			...
			$repeater = false;
			$show     = array_key_exists( 'show', $this->dependency ) ? qode_essential_addons_framework_return_dependency_options_array( $this->scope, $this->type, $this->dependency['show'], true, $repeater ) : array();
			$hide     = array_key_exists( 'hide', $this->dependency ) ? qode_essential_addons_framework_return_dependency_options_array( $this->scope, $this->type, $this->dependency['hide'], false, $repeater ) : array();
		...
		qode_essential_addons_framework_template_part( QODE_ESSENTIAL_ADDONS_ADMIN_PATH, 'inc/common', 'templates/section', $this->get_layout(), $params );
	}
}
